...
# Configs are ~/.config/quickshell/<name> (bin/qs, QS_CONFIG_NAME); a checkout
# still laid out flat, with services/ at the top, is accepted too.
CFG="${QS_CONFIG_ROOT:-$HOME/.config/quickshell/${QS_CONFIG_NAME:-end4}}"
[ -d "$CFG/services" ] || [ -n "${QS_CONFIG_ROOT:-}" ] || CFG="$HOME/.config/quickshell"
QS="${QS_BINARY:-$ROOT/bin/qs}"
