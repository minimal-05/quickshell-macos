...
| `dispatch.mjs` | `translateLuaDispatch` in `shims/Quickshell/Hyprland/Hyprland.qml`, lifted out of the shipped file and run under node: move to a Space, swap two tiled windows (`target`), free-position (`x`/`y`), resize by a corner, focus, close, the bare-move no-op and `null` for an unmapped dispatch | `node tests/dispatch.mjs` |
